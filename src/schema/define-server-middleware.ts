/**
 * API for defining server-side middleware with handler pattern.
 * Server middleware runs on the backend and uses a `next()` chain pattern.
 *
 * @module schema/define-server-middleware
 * @category Schema
 *
 * @example
 * ```typescript
 ...
 * const RequestIdMiddleware = defineServerMiddleware('request-id', {
 ...
 * const ApiKeyMiddleware = defineServerMiddleware('api-key', {
 *   config: {
...
/**
 * Defines server-side middleware with a handler function.
 ...
 *
 * Unlike client middleware (which uses before/after/onError hooks),
 * server middleware uses a handler pattern with `next()` for chaining.
 *
 ...
 * @returns ServerMiddlewareSchema (same as MiddlewareSchema)
 *
 * @example Simple middleware without config
 * ```typescript
 * const LoggerMiddleware = defineServerMiddleware('logger', {
 *   handler: async ({ ctx, next }) => {
 ...
 * ```typescript
 * const CorsMiddleware = defineServerMiddleware('cors', {
 *   config: {
 ...
 * ```typescript
 * const JwtMiddleware = defineServerMiddleware('jwt-auth', {
 *   config: {
 ...
 * ```
 */
export function defineServerMiddleware<TConfig = Record<string, unknown>>(
  name: string,
  options: ServerMiddlewareConfig<TConfig>
): ServerMiddlewareSchema<TConfig> {
  // Validate name
  if (!name || typeof name !== 'string') {
    throw new Error('Server middleware name must be a non-empty string');
  }

  // Validate name format (lowercase, alphanumeric, hyphens)
  if (!/^[a-z][a-z0-9-]*$/.test(name)) {
    throw new Error(
      `Server middleware name must start with a lowercase letter and contain only lowercase letters, numbers, and hyphens: ${name}`
    );
  }

  // Validate handler
  if (typeof options.handler !== 'function') {
    throw new Error(`Server middleware handler must be a function: ${name}`);
  }

  // Normalize config fields
  const config = normalizeFields(options.config);

  // Create the schema object with the `with()` method
  const schema: ServerMiddlewareSchema<TConfig> = {
    name,
    config,
