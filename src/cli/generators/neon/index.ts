...
import { generateEndpointHandlerFile } from './endpoint-handler-template';
import { generateSeedHandlerFile } from './seed-handler-template';
...
export async function generateNeonTarget(
  ...
  if (hasEndpoints) {
    // Generate endpoint type interfaces
    const endpointTypesCode = generateAllEndpointInterfaces(endpoints);
    await writeOutput(join(outputDir, 'endpoint-types.ts'), endpointTypesCode, options.dryRun);
    files.push('endpoint-types.ts');
    console.log('   ✓ endpoint-types.ts');

    // Generate handler file for each endpoint
    for (const endpoint of endpoints) {
      const handlerCode = generateEndpointHandlerFile(endpoint, target, config);
      await writeOutput(join(handlersDir, `${endpoint.name}.ts`), handlerCode, options.dryRun);
      files.push(`handlers/${endpoint.name}.ts`);
      console.log(`   ✓ handlers/${endpoint.name}.ts (${endpoint.method} ${endpoint.path})`);
    }
  }

  // Generate seed handler if production seed is configured
  const hasProductionSeed = shouldGenerateSeedHandler(config);
  if (hasProductionSeed) {
    const seedHandlerCode = generateSeedHandlerFile(allSchemas, target, config);
    await writeOutput(join(handlersDir, '_seed.ts'), seedHandlerCode, options.dryRun);
    files.push('handlers/_seed.ts');
    console.log('   ✓ handlers/_seed.ts (POST /_seed)');
  }

  // Generate main router file (only includes target schemas)
  const routerCode = generateRouterFile(targetSchemas, target, config, endpoints, hasProductionSeed);
  ...
  const indexCode = generateIndexFile(targetSchemas, target, config, customMiddleware, hasEndpoints);
...
function generateIndexFile(
  ...
  if (hasEndpoints) {
    lines.push("export * from './endpoint-types';");
  }
