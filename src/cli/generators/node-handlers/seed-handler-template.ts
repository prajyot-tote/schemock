/**
 * Node.js Seed Handler Template
 *
 * Generates the `handlers/_seed.ts` file for production seeding via HTTP endpoint.
 *
 * @module cli/generators/node-handlers/seed-handler-template
 * @category CLI
 */
...
/**
 * Generate the seed handler file for Node.js targets (Express/Fastify/Hono)
 *
...
export function generateSeedHandlerFile(
  ...
  if (backend === 'supabase') {
    code.line("import { supabase } from '../db';");
  ...
  generateHandlerFunction(code, backend, config);
