...
export function generateMockDb(schemas: AnalyzedSchema[], config: MockAdapterConfig): string {
  ...
  // Use safe property names for JS identifiers
  const entityNames = schemas.map((s) => toSafePropertyName(s.name));
  const persist = config.persistence !== 'memory'; // Default to localStorage

  code.comment('GENERATED BY SCHEMOCK - DO NOT EDIT');
  code.line("import { factory, primaryKey, nullable } from '@mswjs/data';");
  code.line("import { faker } from '@faker-js/faker';");
  code.line("import type { EntitySchema, FieldDefinition, RelationDefinition } from 'schemock/schema';");
  code.line();
  ...
  // Generate EntitySchema array for adapter usage
  generateSchemaExports(code, schemas);
...
/**
 * Generate EntitySchema array export for adapter usage
 * This allows the adapter to access schema metadata for RLS, relations, etc.
 */
function generateSchemaExports(code: CodeBuilder, schemas: AnalyzedSchema[]): void {
  code.comment('Entity schemas for adapter usage');
  code.block('export const schemas: EntitySchema[] = [', () => {
    for (const schema of schemas) {
      generateEntitySchemaObject(code, schema);
    }
  }, '];');
}

/**
 * Generate a single EntitySchema object literal
 */
function generateEntitySchemaObject(code: CodeBuilder, schema: AnalyzedSchema): void {
  code.block('{', () => {
    ...
    code.block('fields: {', () => {
      for (const field of schema.fields) {
        generateFieldDefinitionLiteral(code, field);
      }
    }, '},');

    // Generate relations object if present
    if (schema.relations.length > 0) {
      code.block('relations: {', () => {
        for (const rel of schema.relations) {
          code.block(`${rel.name}: {`, () => {
            code.line(`type: '${rel.type}',`);
            code.line(`target: '${rel.target}',`);
            if (rel.foreignKey) {
              code.line(`foreignKey: '${rel.foreignKey}',`);
            }
            if (rel.through) {
              code.line(`through: '${rel.through}',`);
            }
            if (rel.otherKey) {
              code.line(`otherKey: '${rel.otherKey}',`);
            }
          }, '},');
        }
      }, '},');
    ...
    // Generate RLS config if present
    if (schema.rls.enabled) {
      code.block('rls: {', () => {
        if (schema.rls.scope.length > 0) {
          code.block('scope: [', () => {
            for (const scope of schema.rls.scope) {
              code.line(`{ field: '${scope.field}', contextKey: '${scope.contextKey}' },`);
            }
          }, '],');
        }
        if (schema.rls.bypass.length > 0) {
          code.block('bypass: [', () => {
            for (const bypass of schema.rls.bypass) {
              ...
              code.line(`{ contextKey: '${bypass.contextKey}', values: [${valuesStr}] },`);
            }
          }, '],');
        }
      }, '},');
...
/**
 * Generate a FieldDefinition object literal
 */
function generateFieldDefinitionLiteral(code: CodeBuilder, field: AnalyzedField): void {
  ...
  if (field.refTarget) {
    props.push(`target: '${field.refTarget}'`);
  }
  if (field.enumValues && field.enumValues.length > 0) {
    const enumStr = field.enumValues.map(v => `'${v}'`).join(', ');
    props.push(`values: [${enumStr}] as const`);
  }

  code.line(`${field.name}: { ${props.join(', ')} },`);
}
