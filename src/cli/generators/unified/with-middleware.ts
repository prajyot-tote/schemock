...
export function generateWithMiddleware(
  ...
  const { target, includeJwtExtraction = true, customHeaders = [], middlewareImport } = config;
  ...
  const uniqueMiddleware = collectUniqueMiddleware(schemas);
...
/**
 * Collect all unique middleware referenced by schemas.
 * Deduplicates middleware by name across all schema middleware references.
 *
 * @param schemas - Analyzed schemas to collect middleware from
 * @returns Map of middleware name to unique middleware info
 */
function collectUniqueMiddleware(schemas: AnalyzedSchema[]): Map<string, UniqueMiddleware> {
