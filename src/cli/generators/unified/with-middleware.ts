...
import type { AnalyzedSchema, AnalyzedEndpoint, AnalyzedMiddlewareRef, AnalyzedEndpointMiddleware } from '../../types';
...
export interface WithMiddlewareConfig {
  ...
  /** Path to import user-defined middleware from (e.g., '@/middleware') */
  middlewareImport?: string;
  /** Custom endpoints to generate middleware for */
...
export function generateWithMiddleware(
  ...
  const { target, includeJwtExtraction = true, customHeaders = [], middlewareImport, endpoints = [] } = config;
  ...
  // Generate middleware imports from user-defined middleware
  const uniqueMiddleware = collectUniqueMiddleware(schemas, endpoints);
  generateMiddlewareImports(code, uniqueMiddleware, middlewareImport);
  ...
  generateMiddlewareResolution(code, schemas, endpoints);
...
// ============================================================================
// Middleware Collection & Import Helpers
// ============================================================================

/**
 * Unique middleware entry for deduplication
 */
interface UniqueMiddleware {
  /** Middleware name (e.g., 'auth') */
  name: string;
  /** Import name for code generation (e.g., 'authMiddleware') */
  importName: string;
...
/**
 * Collect all unique middleware referenced by schemas and endpoints.
 * Deduplicates middleware by name across all middleware references.
 *
 * @param schemas - Analyzed schemas to collect middleware from
 * @param endpoints - Analyzed endpoints to collect middleware from
 * @returns Map of middleware name to unique middleware info
 */
function collectUniqueMiddleware(
  schemas: AnalyzedSchema[],
  endpoints: AnalyzedEndpoint[] = []
): Map<string, UniqueMiddleware> {
  const unique = new Map<string, UniqueMiddleware>();

  for (const schema of schemas) {
    if (schema.isJunctionTable) continue;

    // Collect from default middleware
    for (const ref of schema.middleware ?? []) {
      if (!unique.has(ref.name)) {
        unique.set(ref.name, {
          name: ref.name,
          importName: `${ref.name}Middleware`,
          sourceFile: ref.sourceFile ?? ref.middleware?.sourceFile,
        });
      }
    }

    // Collect from per-operation middleware
    const ops = ['list', 'get', 'create', 'update', 'delete'] as const;
    for (const op of ops) {
      for (const ref of schema.endpointMiddleware?.[op] ?? []) {
        if (!unique.has(ref.name)) {
          unique.set(ref.name, {
            name: ref.name,
            importName: `${ref.name}Middleware`,
  ...
  // Collect from custom endpoints
  for (const endpoint of endpoints) {
    for (const ref of endpoint.middleware ?? []) {
      if (!unique.has(ref.name)) {
        unique.set(ref.name, {
          name: ref.name,
          importName: `${ref.name}Middleware`,
...
/**
 * Generate import statements for user-defined middleware.
 * Groups imports by source file or uses configured middlewareImport path.
 *
 * @param code - Code builder to write to
 * @param uniqueMiddleware - Map of collected unique middleware
 * @param middlewareImport - Optional configured import path
 */
function generateMiddlewareImports(
  code: CodeBuilder,
  uniqueMiddleware: Map<string, UniqueMiddleware>,
  middlewareImport?: string
): void {
  if (uniqueMiddleware.size === 0) return;

  // Group by source file (or use configured middlewareImport)
  const importNames = Array.from(uniqueMiddleware.values())
    .map(mw => mw.importName);

  if (middlewareImport) {
    // Use configured path for all middleware
    code.line(`import { ${importNames.join(', ')} } from '${middlewareImport}';`);
  } else {
    // Group by source file
    const bySourceFile = new Map<string, string[]>();
    const noSource: string[] = [];

    for (const [, mw] of uniqueMiddleware) {
      if (mw.sourceFile) {
        const existing = bySourceFile.get(mw.sourceFile) ?? [];
        existing.push(mw.importName);
        bySourceFile.set(mw.sourceFile, existing);
      } else {
        noSource.push(mw.importName);
      }
    }

    for (const [path, imports] of bySourceFile) {
      code.line(`import { ${imports.join(', ')} } from '${path}';`);
    }

    if (noSource.length > 0) {
      code.comment(`TODO: Import middleware without sourceFile: ${noSource.join(', ')}`);
    }
  }
  code.line();
}
...
/**
 * Generate middleware array expression from refs.
 * Handles .with() config overrides for inline configuration.
 *
 * @param refs - Array of middleware references
 * @returns JavaScript array literal string (e.g., '[authMiddleware, rateLimitMiddleware.with({"max":10})]')
 */
function generateMiddlewareArray(refs: AnalyzedMiddlewareRef[] | undefined): string {
  if (!refs || refs.length === 0) return '[]';

  const items = refs.map(ref => {
    const importName = `${ref.name}Middleware`;
    if (ref.hasConfigOverrides && ref.configOverrides) {
      // Generate .with() call inline
      return `${importName}.with(${JSON.stringify(ref.configOverrides)})`;
    }
    return importName;
  });
...
// ============================================================================
// Middleware Resolution Generation
// ============================================================================

/**
 * Generate middleware resolution helper
 */
function generateMiddlewareResolution(
  code: CodeBuilder,
  schemas: AnalyzedSchema[],
  endpoints: AnalyzedEndpoint[] = []
): void {
  ...
  // Collect endpoint middleware configs
  const endpointConfigs = new Map<string, AnalyzedMiddlewareRef[]>();
  for (const endpoint of endpoints) {
    endpointConfigs.set(endpoint.name, endpoint.middleware ?? []);
  }
  ...
  // Generate endpoint middleware resolution function
  code.multiDocComment([
    'Get middleware handlers for a custom endpoint.',
    '',
    '@param endpointName - Custom endpoint name (e.g., "search")',
    '@returns Array of middleware handlers to run',
  ]);
  code.block('export function getEndpointMiddleware(endpointName: string): MiddlewareHandler[] {', () => {
    code.line('return endpointMiddlewareConfig[endpointName] ?? [];');
  });
  code.line();

  // Generate middleware configuration map with actual middleware
  ...
  for (const schema of schemas) {
    ...
    code.block(`${schema.name}: {`, () => {
      // Generate actual default middleware array
      code.line(`default: ${generateMiddlewareArray(config.default)},`);

      code.block('operations: {', () => {
        // Generate per-operation middleware overrides
        for (const op of ['list', 'get', 'create', 'update', 'delete'] as const) {
          const opMiddleware = config.perOperation[op];
          if (opMiddleware !== undefined) {
            // Explicit per-operation override (could be empty array for public endpoints)
            code.line(`${op}: ${generateMiddlewareArray(opMiddleware)},`);
          }
        }
      }, '},');
    }, '},');
  ...
  // Generate endpoint middleware configuration map
  code.comment('Custom endpoint middleware configuration');
  code.block('const endpointMiddlewareConfig: Record<string, MiddlewareHandler[]> = {', () => {
    for (const endpoint of endpoints) {
      const middleware = endpointConfigs.get(endpoint.name) ?? [];
      code.line(`${endpoint.name}: ${generateMiddlewareArray(middleware)},`);
    }
  }, '};');
}
