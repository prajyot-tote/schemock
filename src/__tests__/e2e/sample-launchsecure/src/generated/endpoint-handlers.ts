...
export const endpointHandlers = [
  ...
  http.get('/api/authhub/users', async ({ request, params: pathParams }) => {
    ...
    try {
      const result = await endpointResolvers.authhubUsers({ db, params: params as Types.AuthhubUsersParams, body: body, headers, context });
  ...
  http.get('/api/auth/user-permissions', async ({ request, params: pathParams }) => {
    const params = {};
    ...
    try {
      const result = await endpointResolvers.authUserPermissions({ db, params: params, body: body, headers, context });
      return HttpResponse.json(result);
    } catch (error) {
      return handleError(error);
    }
  ...
  http.post('/api/auth/invite-user', async ({ request, params: pathParams }) => {
    ...
    try {
      const result = await endpointResolvers.authInviteUser({ db, params: params as Types.AuthInviteUserParams, body: body, headers, context });
      return HttpResponse.json(result);
    } catch (error) {
      return handleError(error);
    }
