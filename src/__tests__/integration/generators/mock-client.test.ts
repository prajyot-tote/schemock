...
describe('Generated Code Compilation', () => {
  ...
  beforeAll(async () => {
    tempDir = await createTempDir('mock-client-compile-');
  });
  ...
  it('mock client compiles without TypeScript errors', async () => {
    const analyzed = analyzeTestSchemas(simpleSchemas);
  ...
  it('mock client with RLS compiles without errors', async () => {
    const analyzed = analyzeTestSchemas(rlsSchemas);
